...
terraform/
├── main.tf                 # Providers, SSH key gen, firewall (SSH + Syncthing), Hetzner server, outputs
├── variables.tf            # Input vars (hcloud_token, API keys, server config)
├── user-data.yml           # Cloud-init: user, packages, Node.js 22, gcloud, OpenClaw, Syncthing, hardening
