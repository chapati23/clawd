...
scripts/
├── lib.sh                  # Shared helpers: colors, logging, GPG/pass utils, tfvars helpers
├── credentials-init.sh     # One-time: bootstrap pass + GPG + age + GitHub repo (MacBook)
├── add-bot.sh              # Per-bot: GPG key + credential scope + deploy key (MacBook)
...
├── variables.tf            # Input vars (hcloud_token, API keys, server config)
├── user-data.yml           # Cloud-init: user, packages, Node.js 22, gcloud, OpenClaw, Syncthing, Tailscale, hardening
├── terraform.tfvars        # Secrets (gitignored, auto-generated by setup.sh)
└── .gitignore
...
4. Resolve tokens (pass → env → tfvars → interactive prompt)
...
1. `make dashboard-setup` — configures gateway: trustedProxies, allowedOrigins, allowTailscale, tailscale.mode
2. Open the dashboard in your browser (the URL is printed by the command above)
3. You'll see "pairing required" — this is expected on first connect
4. `make dashboard-pair` — approves your browser as a paired device
5. Reload the browser — the dashboard should connect
...
3. Run `./setup.sh` — the key is resolved automatically and the server joins your tailnet on boot
...
- **No direct linter invocation** — always use `trunk check` / `trunk fmt`

### Shell script patterns

- Use `lib.sh` helpers: `step()`, `info()`, `ok()`, `warn()`, `error()` for logging
- Use `pass_get()`, `pass_entry_exists()`, `gpg_key_id()`, `gpg_key_exists()` — don't reinvent
- Use `tfvars_get()` / `tfvars_update()` for safe terraform.tfvars manipulation
- Temporary sensitive files: use `mktemp` + `trap 'rm -f' EXIT`
- Scripts that run on server can't source `lib.sh` — inline the logging helpers
...
- ✅ **Do:** Add scripts, modify cloud-init, update Terraform resources, add Checkov policies, update credential management
- ✅ **Do:** Use `lib.sh` helpers in new scripts
- ⚠️ **Ask first:** Changing firewall rules, modifying SSH hardening, adding new open ports, changing credential store layout
- 🚫 **Never:** Store secrets in git, run raw `hcloud` CLI for infra changes, modify `terraform.tfvars` manually (use pass + setup.sh), disable security policies without justification
